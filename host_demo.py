...
DEFAULT_CONFIG = {
    ...
    "vmware_title_keyword": "VMware Workstation",
    "delays": {
        "after_tab": 0.7,
        "after_verify": 4.0,
...
def find_vmware_window(keyword: str):
    ...
    if not wins:
        raise RuntimeError(f"找不到包含“{keyword}”的 VMware 窗口。")
...
class HostDemo(tk.Tk):
    def __init__(self):
        ...
        self.title("MusicVMAuto Host Demo - QQ音乐")
    ...
    def _build(self):
        ...
        ttk.Label(root, text="QQ音乐 VMware 宿主机自动化 Demo", font=("Microsoft YaHei UI", 16, "bold")).pack(anchor="w")
        ttk.Label(root, text="只运行在 VMware 外面的宿主机；虚拟机里不安装 Python、不复制任何文件。").pack(anchor="w", pady=(4, 12))
        ...
        ttk.Button(cfgf, text="保存配置", command=self.save_basic).grid(row=0, column=2, rowspan=2, padx=12)

        cal = ttk.LabelFrame(root, text="A-1 首次校准（坐标相对 VMware 窗口保存）", padding=10)
        ...
        for row, (key, label) in enumerate(items):
            ...
            ttk.Button(cal, text="测试点击", command=lambda k=key: self.run_bg(lambda: self.test_click(k))).grid(row=row, column=3, padx=6)
        ...
        note = ttk.LabelFrame(root, text="当前 Demo 的边界", padding=10)
        note.pack(fill="x", pady=(0, 10))
        ttk.Label(note, text="现在先验证宿主机能否稳定点击 A-1 里的关键位置。IP 的 √ 识别、QQ音乐弹窗识别、A-1~A-50 自动切换会在这个基础上继续加。", wraplength=760).pack(anchor="w")
    ...
    def save_basic(self):
        ...
        self.cfg["vmware_title_keyword"] = self.vmware_var.get().strip() or "VMware Workstation"
    ...
    def _capture_worker(self, key, label):
        ...
        self.log(f"已记录 {label}：相对位置 ({rx:.4f}, {ry:.4f})")
    ...
    def _click(self, key, clicks=1, interval=0.15):
        win = find_vmware_window(self.cfg["vmware_title_keyword"])
        x, y = point_to_screen(win, self.cfg["points"].get(key))
        pyautogui.click(x, y, clicks=clicks, interval=interval)
        self.log(f"点击 {key}: ({x}, {y})")
    ...
    def test_playlist(self):
        ...
        self._click(f"playlist_{no}", clicks=2)
